...
function bindClickForJobListing() {
    document.querySelectorAll('.crr-joblisting-wrapper').forEach(item => {
        item.addEventListener('click', event => {
            ...
            document.getElementById('hd_i').value = href.split('/').slice(-1)[0];
            var params = getPostFilterConditionParams();
            submitDynamicFilterForm(href, params);   
        })
    });
    ...
    let loadMoreEleFromHttpPost = document.getElementById('crr-loadmore-httppost');
...
function seeMoreResults(obj) {
    ...
    $(".crr-find-job-wrapper,.crr-column-left,.crr-filter-title").show();
...
function loadJob(obj) {
    ...
    $.ajax({
        url: CRR_REQUESTDATA_API.GetJobByCareerJobPostingID,
        data: {
            strJobPostingID: pid,
            strLocation: $("#hd_l").val(),
            strBenchmarkJobCode: $("#hd_BenchmarkJobCode").val(),
            strSkillCodesFromJobDesc: $("#hd_SkillCodesFromJobDesc").val(),
            strCleanJobPostingTitle: $("#hd_CleanJobPostingTitle").val()
        },
        type: "post",
        cache: false,
        beforeSend: function () {
        ...
        success: function (data) {
            $(".crr-column-right").html(data);

            if (!$("#hd_pid").val()) {
                history.replaceState(null, null, location.href + "&pid=" + pid);
            }
            else {
                history.replaceState(null, null, location.href.replace("&pid=" + $("#hd_pid").val(), "&pid=" + pid));
            }
            $("#hd_pid").val(pid);

            if ($(".crr-joblisting-wrapper.cursor-pointer.job.selected").length) {
                $(".crr-joblisting-wrapper.cursor-pointer.job.selected").removeClass("job").removeClass("selected");
            }
            $(obj).addClass("job").addClass("selected");
        }
    });
